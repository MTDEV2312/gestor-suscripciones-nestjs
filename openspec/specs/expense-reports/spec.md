...
## 1. Overview
The `expense-reports` capability provides aggregated expense analytics, multi-currency consolidated summaries, monthly spending breakdowns, and dual-format exports (Excel-compatible UTF-8 BOM CSV and executive-styled PDF) for subscription payments.
...
| **REQ-ER-05** | Reports Navigation & Format Selection | **SHALL** | Include a navbar link to "Reportes" (`/reports`) offering period presets ("Este Mes", "Mes Anterior", "Trimestre", "Año Actual", "Personalizado"), summary metric cards, Recharts chart, and an export dropdown menu supporting both PDF and CSV formats with loading indicators. |
...
### Scenario 4: Generate executive PDF expense report (PDF Export Path)
- **Given** an authenticated user with payments in the selected period
- **When** the user sends `GET /payments/report/export/pdf?startMonth=2026-01&endMonth=2026-08&targetCurrency=USD`
- **Then** the system MUST return HTTP `200 OK` with `Content-Type: application/pdf` and `Content-Disposition: attachment; filename="reporte-gastos-2026-01.pdf"`
- **And** the PDF document MUST render the header banner, metadata box, KPI cards, subscription breakdown summary, monthly breakdown, detailed transactions table with payment methods and status pills, and dynamic pagination footer.

### Scenario 5: Filter expenses by specific subscription (Filter Edge Case)
...
- **And** the PDF export MUST display the highlighted "INFORMACION DE LA SUSCRIPCION" profile card.

### Scenario 6: Reject invalid date query parameters (Validation Error)
